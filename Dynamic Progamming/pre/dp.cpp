...
/* Longest Increasing Subsequence 👏 👏 👏 
    1. lot of error in tab and memo code.
    2. see tab coded */
    // brute
        class Solution {
        public:
            int pre(int ind,int prev_ind,vector<int>& nums){
                if(ind == nums.size()) return 0;
                
                int dtake = 0,take = 0;
                dtake = 0 + pre(ind +1,prev_ind,nums);
                if(prev_ind == -1 or nums[ind] > nums[prev_ind])
                    take = 1 + pre(ind +1,ind,nums);
    ...
    // memo
        class Solution {
        public:
            int pre(int ind,int prev_ind,vector<int>& nums,vector<vector<int>>& dp){
                if(ind == nums.size()) return 0;
                if(dp[ind][prev_ind + 1] != -1) return dp[ind][prev_ind + 1];
                
                int dtake = 0,take = 0;
                dtake = 0 + pre(ind +1,prev_ind,nums,dp);
                if(prev_ind == -1 or nums[ind] > nums[prev_ind])
                    take = 1 + pre(ind +1,ind,nums,dp);
                
                return dp[ind][prev_ind + 1] = max(take,dtake);
            }
            int lengthOfLIS(vector<int>& nums) {
                int n = nums.size();
                vector<vector<int>> dp(n+1,vector<int> (n+1,-1));
                return pre(0,-1,nums,dp);
            }
        };
    ...
    // tab
        ...
        public:
            int lengthOfLIS(vector<int>& nums) {
                ...
                for(int ind=n-1;ind>=0;ind--){
                    for(int prev_ind=ind-1;prev_ind>=-1;prev_ind--){ // n - 1 => ind -1 ⭐
                        int dtake = 0, take = 0;
                        dtake = 0 + dp[ind+1][prev_ind+1]; // prev_ind => prev_ind ⭐
                        if(prev_ind==-1 or nums[ind] > nums[prev_ind])
                            take = 1 + dp[ind+1][ind+1]; // ind => ind + 1 ⭐
                        dp[ind][prev_ind+1] = max(take,dtake);
                    }
                }
                
                return dp[0][0]; // dp[0][-1] => d[0][-1+1] ⭐
            }
        };
            ...
            int lengthOfLIS(vector<int>& nums) {
                ...
                for(int ind=n-1;ind>=0;ind--){
                    for(int prev_ind=ind-1;prev_ind>=-1;prev_ind--){
                        int dtake = 0, take = 0;
                        dtake = 0 + dp[prev_ind+1];
                        if(prev_ind==-1 or nums[ind] > nums[prev_ind])
                            take = 1 + dp[ind+1]; 
                        curr[prev_ind+1] = max(take,dtake);
                    }
                    dp = curr;
                }
...
// Longest Bitonic Sequence , this code is working only in coding ninja not in leetcode
    #include <bits/stdc++.h> 
    int longestBitonicSequence(vector<int>& nums, int n) {
        ...
        for(int i=n-1;i>=0;i--){
            for(int prev=n-1;prev>i;prev--){ // prev = 0 ; prev < i
                if(nums[prev] < nums[i]){
...
// Longest Bitonic Sequence , we have to find the number of deletion , I have not read the question properly
    class Solution {
    public:
...
// Todo: Number of Longest Increasing Subsequence
    // brute    
        class Solution {
        public:
...
// Matrix Chain Multiplication
    // brute
        class Solution{
        public:
            int pre(int i,int j,int arr[],int N){
                if(i == j) return 0;
                int mini = INT_MAX;
                for(int k  = i;k<j;k++){
                    int steps = arr[i-1]*arr[k]*arr[j] + pre(i,k,arr,N) + pre(k+1,j,arr,N);
                    mini = min(mini,steps);
                }
                return mini;
            }
    ...
    // memo
        class Solution{
        public:
            int pre(int i,int j,int arr[],int N,vector<vector<int>>& dp){
                if(i == j) return 0;
                if(dp[i][j] != -1) return dp[i][j];
                
                int mini = INT_MAX;
                for(int k  = i;k<j;k++){
                    int steps = arr[i-1]*arr[k]*arr[j] + pre(i,k,arr,N,dp) + pre(k+1,j,arr,N,dp);
                    mini = min(mini,steps);
                }
                return dp[i][j] = mini;
            }
...
// Parsing A Boolean Expression or Boolean Evaluation or Evaluate Boolean Expression to True
    // brute
        const int32_t M = 1e9+7;
        #include <bits/stdc++.h> 

            int pre(int i,int j,bool isTrue,string &s){
                if(i > j) return 0;
                if(i == j){
                ...
                long ways = 0;
                for(int ind=i+1;ind<=j-1;ind+=2){ // ind++ , ind < j
                    long lt = pre(i,ind-1,1,s);
                    long lf = pre(i,ind-1,0,s);
                    long rt = pre(ind+1,j,1,s);
                    long rf = pre(ind+1,j,0,s);
                    
                    if(s[ind] == '&'){
                        if(isTrue) ways = (ways + (lt*rt)%M)%M;
                        else ways = (ways + ((lt*rf)%M + (lf*rt)%M + (lf*rf)%M)%M)%M;
                    }else if(s[ind] == '|'){
                        if(isTrue) ways = (ways + ((lt*rt)%M + (lt*rf)%M + (lf*rt)%M)%M)%M;
                        else ways = (ways + ((lf*rf)%M))%M;
                    }else if(s[ind] == '^'){
                        if(isTrue) ways = (ways + ((lt*rf)%M + (lf*rt)%M)%M)%M;
                        else ways = (ways + ((lt*rt)%M + (lf*rf)%M)%M)%M;
                    }
                }
                return ways;
            }
        int evaluateExp(string & exp) {
            int n = exp.size();
            return pre(0,n-1,true,exp);
        }

    // memo
        const int32_t M = 1e9+7;
        #include <bits/stdc++.h> 
        int pre(int i,int j,bool isTrue,string &s,vector<vector<vector<int>>>& dp){
            if(i > j) return 0;
            if(i == j){
                if(isTrue) return s[i] == 'T';
                else return s[i] == 'F';
            ...
            long ways = 0;
            for(int ind=i+1;ind<=j-1;ind+=2){ // ind++ , ind < j
                long lt = pre(i,ind-1,1,s,dp);
                long lf = pre(i,ind-1,0,s,dp);
                long rt = pre(ind+1,j,1,s,dp);
                long rf = pre(ind+1,j,0,s,dp);
...
// Palindrome Partitioning II
        ...
        public:
            ...
            int pre(int i,int n,string& s){
                ...
                for(int ind=i;ind<n;ind++){
                    ...
                    if(palindrome(temp)) 
                        mini = min(mini,1+pre(ind+1,n,s)); // i + 1
                }
                return mini;
            }
            
            int minCut(string s) {
                return pre(0,s.size(),s) - 1; // forgot subtracting -1
            }
        };
    ...
    // memo
        ...
        public:
            ...
            int pre(int i,int n,string& s,vector<int>& dp){
                if(i == n) return 0;
                ...
                for(int ind=i;ind<n;ind++){
                    temp.push_back(s[ind]);
                    if(palindrome(temp)) 
                        mini = min(mini,1+pre(ind+1,n,s,dp)); // i + 1
                }
                return dp[i] = mini;
            }
            ...
            int minCut(string s) {
                int n = s.size();
                vector<int> dp(n+1,-1);
                return pre(0,n,s,dp) - 1; // forgot subtracting -1
            }
        };
        ...
        public:
            ...
            int minCut(string s) {
                ...
                for(int i=n-1;i>=0;i--){
                    ...
                    for(int ind=i;ind<n;ind++){
                        temp.push_back(s[ind]);
                        if(palindrome(temp)) mini = min(mini,1+ dp[ind+1]);
                    }
                    dp[i] = mini;
                }
    ...
    // tab
        ...
        public:
            vector<vector<string>> groupAnagrams(vector<string>& strs) {
                ...
                vector<string> ans(strs), set<string> st;;
